...
# from models.Fed import FedAvgMask, FedCompare, compute_dissimilarity, pia_difference
...
if __name__ == '__main__':
    ...
    args.epsilon = 0.005 if args.defense is not None else None########None  # 1##
    args.frac = 0.1  ###0.1#####client num
    args.no_mask = False  # True  # #### #是否用fedmask训练
    args.num_users = 100  # 0  ## 2  #200#
    args.epochs = 100  # 150## # ###30
    args.local_ep = 20#1###5####15##100  #70  # 5#10
    args.thrs = 0.7 if not args.no_mask else 0.5
    ...
    args.data_name = 'mnist'  #'cifar10'# 'adult' #'CelebA'#'lfw'###  ####
    args.model = 'cnn'#'cnn'#'cnnlfw'###'fc'  # 'vgg16'###'resnet'##### ###
    args.EPS = 1  #######0######0.2##0.1 0.5 ### 0.7#####0.2###top k  0.1  # 0.5  # 0.05  # 0.7##.7  # mask updating threshold 0.9 # 99%更新，1%不变
    torch.random.manual_seed(42)
    torch.cuda.manual_seed(42)
    torch.cuda.manual_seed_all(42)
    epsilon = None  # 2 LDP epsilon
    ...
    if args.data_name == 'CelebA' or args.data_name == 'lfw':  # 指定数据集
        args.num_items_train = 150  # 300 args.num_samples  # 20, number of local data size
        transform = transforms.Compose([
            transforms.Resize([64, 64]),##### if args.model=='cnnlfw' else None,
    ...
    if (args.data_name == 'mnist' or args.data_name == 'cifar10'):  #### and not args.iid
        dict_users, dict_users_test, prop = mnist_extr_noniid(data1, testdataset, args.num_users + 1, args.n_class,
                                                              args.num_items_train, args.unbalance_rate)
        prop = [1 for _ in range(len(dict_users))]
        attack_type=None
        print('non-iid', args.data_name,prop)
    else:  # prop
        attack_type = 'pia'
    ...
    if args.model == 'cnn':
        ...
        args.affine = True
        if not args.no_mask:
            net_original = ConvNet(width=64, num_channels=3, num_classes=args.num_classes).to(args.device)
    ...
    filename = '{}_{}_selected_{}_iid_{}_{}_{}_{}_{}_EPS_{}_{}_{}_poison_{}_frac_{}_prop_rate_{}_local_ep{}_thr{}.txt'.format(
        args.optim, args.no_mask, args.tau, args.iid, args.model, args.data_name, target, privacy, args.EPS,
        args.defense, args.epsilon,
        ...
        if not args.no_mask:  ######fedmask
            for task in range(1):
                ...
                for idx in idxs_users:  # prop[idx] = task
                    ...
                    mask_real_local, masks, sensitivity, local_acc = local.train(args.local_ep, epsilon=epsilon,
                                                                                 sen=sensitivity,
                ...
                net_glob_list[task], _, mask_avg, _, _ = FedCompare(net_glob_list[task], mask_locals, prop,
                                                                    mask_ori, epsilon=0, layer_wise=args.layer_wise,threshold=args.thrs*(1+iter/args.epochs))
                mask_diff.extend(pia_difference(net_glob_list[task], mask_locals,
                                                mask_ori))  # [client1{layer1:[],layer2:[],...},client2{},...]
            ...
            for task in range(1):
                if iter == 1:
                    ...
                    layerid = []
                    for module_idx, module in enumerate(w_avg.keys()):
                        if 'weight' in module:
    ...
    if args.data_name not in ['mnist','cifar10']:
        if not args.no_mask:  ##fedmask
            acc = pia_mask(layerid, None, None, args=args, target=target, privacy=privacy, trainset=mask_diff,
                           train_prop=label_prop, iter=iter, clients_num=args.num_users, m=m)
